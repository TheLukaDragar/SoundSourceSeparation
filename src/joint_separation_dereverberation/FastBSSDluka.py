...
from Base_luka import EPS, MIC_INDEX, Base, MultiSTFT,MultiISTFT
...
class FastBSSD(Base):
    ...
    def separate_og(self, mic_index=MIC_INDEX):
        self.calculate_Yn()
        self.calculate_Y()
        self.calculate_Px()
    ...
    def separate(self):
        ...
        separated_spec_all_mics = np.zeros([self.n_mic, self.n_speech, self.n_freq, self.n_time], dtype=np.complex128)
        
        for mic in range(self.n_mic):
            for n in range(self.n_speech):
                tmp = (Q_inv_FMM[:, None, mic] * (self.Y_NFTM[n] / self.Y_FTM * self.Px_FTM)).sum(axis=2)
                separated_spec_all_mics[mic, n] = self.convert_to_NumpyArray(tmp)
        
        self.separated_spec_all_mics = separated_spec_all_mics
        return self.separated_spec_all_mics
    ...
    def save_to_wav(self, spec, save_fname="./sample.wav", shape="FTM"):
        ...
        if spec.ndim == 4:  # If handling all microphones
            for mic in range(spec.shape[0]):
                separated_signal = MultiISTFT(spec[mic].transpose(1, 2, 0), shape=shape)
                mic_save_fname = save_fname.replace(".wav", f"_mic{mic}.wav")
                print(f"Saving {mic_save_fname}")
                sf.write(mic_save_fname, separated_signal, self.sample_rate)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_iter", type=int, default=150, help="number of iterations")
    parser.add_argument("--init_SCM", type=str, default="twostep", help="circular or twostep")
    parser.add_argument("--algo", type=str, default="ISS2", help="IP or ISS")
    parser.add_argument("--n_tap_MA", type=int, default=8, help="filter length for MA model")
    parser.add_argument("--n_tap_AR", type=int, default=4, help="filter length for AR model")
    parser.add_argument("--n_delay_AR", type=int, default=3, help="delay parameter for AR model")
    ...
    # Only for DNN source model
    parser.add_argument("--n_iter_z", type=int, default=10, help="number of iterations for updating Z")
    ...
    parser.add_argument("--n_bit", type=int, default=64, help="number of bit for float and complex")
    parser.add_argument("--g_eps", type=float, default=1e-2, help="initial value of non-diagonal element of G_NM")
    parser.add_argument("--n_iter_init", type=int, default=30, help="number of iterations for twostep initialization")
    parser.add_argument("--interval_norm", type=int, default=10, help="interval of normalization")

    parser.add_argument("--chunk_duration",  type=int, default=10)
    ...
    chunk_duration = args.chunk_duration  # duration of each chunk in seconds
    ...
    separated_chunks = []
    c = 0
    for start in tqdm(range(0, len(wav), chunk_size), desc=f"Processing chunks"):


        M = min(len(wav), args.n_mic)
        end = min(start + chunk_size, len(wav))
        chunk = wav[start:end, :M]

        
      
        spec_FTM = MultiSTFT(chunk, n_fft=args.n_fft)
        ...
        separater.load_spectrogram(spec_FTM, sample_rate=sample_rate)
        separater.solve(
            n_iter=args.n_iter,
        ...
        out = separater.separate()

        separated_chunks.append(out)

        #save to output

        otpit_dir = f"{str(Path(os.path.abspath(__file__)).parents[2])}/output_chunks/"
        os.makedirs(otpit_dir, exist_ok=True)

        f= os.path.join(otpit_dir, os.path.basename(args.input_fname).replace(".wav", f"_{c}.wav"))

        c+=1

        separater.save_to_wav(out, save_fname=f, shape="FTM")

        print(f"out shape: {out.shape}")

        combined_chunks = np.concatenate(separated_chunks,axis=3)

        print(f"combined_chunks shape: {combined_chunks.shape}")


        otpit_dir = f"{str(Path(os.path.abspath(__file__)).parents[2])}/output/" 
        os.makedirs(otpit_dir, exist_ok=True)

        f= os.path.join(otpit_dir, os.path.basename(args.input_fname).replace(".wav", f"_.wav"))

        separater.save_to_wav(combined_chunks, save_fname=f, shape="FTM")
